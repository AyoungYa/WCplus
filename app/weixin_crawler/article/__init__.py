        ...
        while not requests_done:
            try:
                ...
                if '访问过于频繁，请用微信扫描二维码进行访问' in r_text or '<title>验证</title>' in r_text or 'IP Address:' in r_text:
                    ...
                    if save_html == 'true':
                        DecodeArticle.save_html_as_file(nickname=nickname, file_name=article_data['id'],
                                                        html_str=r_text)
...
class WorkerService:
    ...
    def init_proxy_ips(self, ip_num=1, proxy=False):
        ...
        for ip in ips:
            ...
            TaskRecoder.add_ip_log(ip=ip, l1l11lllll_wcplus_=time.time())
...
class TaskRecoder:
    ...
    def update_ip_log(ip):
        ...
        ts['ips'][ip]['speed'] = round(
            (ts['ips'][ip]['last_time'] - ts['ips'][ip]['created_time']) / ts['ips'][ip]['used_unm'], 3)
        ts['done_task_num'] += 1
...
def get_all_article(worker_num=128, process=None):
    ...
    rc = RequestContent()
    task_num = rc.prepare_articles(nickname, worker_num=worker_num, ip_num=1,
