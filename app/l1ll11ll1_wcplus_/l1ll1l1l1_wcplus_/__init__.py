    ...
    def get_all_reading_data(self, process=None, mov=10):
        ...
        if 'getappmsgext' in self.wx_req_data_list[0]:
            ...
            for a in self.articles:
                ...
                self.pre_crawl_time = time.time()
                reading_data = Crawler(a[1], a[2], self.wx_req_data_list[a[0] % self.wx_num]).run()
                reading_data = self.check(reading_data, a)
                reading_data['id'] = get_md5(a[1])
                self.col_data.insert('id', reading_data)
                process.new_reading_data(a[0] + 1, len(self.articles), self.delay)
